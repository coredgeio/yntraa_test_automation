...
"""Verify the volume type is a mandatory field"""
@pytest.mark.testrail(65777)
def test_verify_volume_type_is_a_mandatory_field(page, ananta_setup):
    vol_type = page.locator(locators['VOL_TYPE']).inner_text()
    assert vol_type == StorageTextData.volume_type, "Aesterisk is not present making Volume Type not mandatory"
    logging.info("Volume type is mandatory!")
...
def test_verify_volume_size_is_a_multiple_of_50(page, ananta_setup):
    if page.locator(locators['VOL_SIZE']).clear():
       ...
       assert vol_50 == StorageTextData.vol_size_50, "column size is not multiple of 50"
       page.wait_for_timeout(TIMEOUT)
...
def test_Attach_it_to_a_Virtual_machine_swipe_button_functionality(page, ananta_setup):
    ...
    expect(swipe_btn).to_be_visible()
    if swipe_btn.click():
        select_a_vm = page.locator(locators['SELECT_VM_TEXT']).inner_text()
        assert select_a_vm == StorageTextData.select_vm, "please select a VM functionality is not present"
...
def test_create_button_becomes_enable_once_required_fields_have_values(page, ananta_setup):
    ...
    if volume_name_field == vol_name_text_placeholder:
        ...
        vol_size.type('50')
        page.get_by_test_id(locators['VOL_SWIPE_BUTTON']).click()
        assert page.get_by_test_id(locators['V0L_CONFIRM_BTN']).is_enabled()
    page.get_by_test_id(locators['VOL_CROSS_BTN']).click()
...
def test_created_status_against_volume(page, ananta_setup):
    page.get_by_test_id(locators['CREATE_VOL_BUTTON']).click()
    page.wait_for_timeout(TIMEOUT)
    page.locator(locators['VOL_NAME']).type('Storage2')
    page.get_by_test_id(locators['V0L_CONFIRM_BTN']).click()
    page.wait_for_timeout(TIMEOUT)
...
"""Verify volume status when vm is attached"""
@pytest.mark.testrail(65134)
def test_volume_status_when_vm_is_attached(page, ananta_setup):
    page.get_by_test_id(locators['CREATE_VOL_BUTTON']).click()
    page.wait_for_timeout(TIMEOUT)
    page.locator(locators['VOL_NAME']).type('Storage3')
    page.get_by_test_id(locators['VOL_SWIPE_BUTTON']).click()
    page.get_by_test_id(locators['ARROW_BTN']).click()
    page.wait_for_timeout(TIMEOUT)
    dropdown_elements = page.query_selector_all(f'[data-testid="compute-id-select-option"]')
    ...
    page.wait_for_timeout(10000)
    page.get_by_test_id(locators['BACK_BTN']).click()
    attached_status = page.locator(locators['ATTACHED_STATUS']).inner_text()
    page.wait_for_timeout(TIMEOUT)
    assert attached_status == StorageTextData.attached_status, "vm is not attached"
    logging.info("Volume is created")
